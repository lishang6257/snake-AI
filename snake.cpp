...
void Snake::updateSBuffStatus()
{
    for(int i = 0;i < SBuffStatus.size();i ++){
        if(gameTime - SBuffStatus[i].second >= Map_SStatusTime.find(SBuffStatus[i].first)->second){
            //处理幽灵态
    ...
    */
    ...
    switch(f[nhead]){
        ...
        case SNAKE                :{
            ...
            for(int i = 0;i < ss.size();i ++){
                //自掘坟墓
                if(ss[i].getID() == id) return move(f);
                //死蛇 || 幽灵蛇 跳过
                if(!ss[i].isAlive() || ss[i].isInvisible()) continue;
                //其他
...
bool Snake::launchWeapon(Field& f)
{
    ...
    position weaponPos = asnake[0] + Direction[dir];
    weaponPos.currect();
